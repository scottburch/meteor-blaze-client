...
var Connection = function (url, options) {                                                                        // 12
  ...
  // collection -> IdMap of "server document" object. A "server document" has:                                    // 96
...
_.extend(Connection.prototype, {                                                                                  // 376
  ...
  _retrieveAndStoreOriginals: function (methodId) {                                                               // 757
    ...
    _.each(self._stores, function (s, collection) {                                                               // 763
      ...
      // not all stores define retrieveOriginals                                                                  // 765
      if (!originals)                                                                                             // 766
        return;                                                                                                   // 767
      originals.forEach(function (doc, id) {                                                                      // 768
        docsWritten.push({collection: collection, id: id});                                                       // 769
        if (!_.has(self._serverDocuments, collection))                                                            // 770
          self._serverDocuments[collection] = new LocalCollection._IdMap;                                         // 771
        var serverDoc = self._serverDocuments[collection].setDefault(id, {});                                     // 772
        if (serverDoc.writtenByStubs) {                                                                           // 773
          // We're not the first stub to write this doc. Just add our method ID                                   // 774
          // to the record.                                                                                       // 775
          serverDoc.writtenByStubs[methodId] = true;                                                              // 776
        } else {                                                                                                  // 777
          // First stub! Save the original value and our method ID.                                               // 778
          serverDoc.document = doc;                                                                               // 779
          serverDoc.flushCallbacks = [];                                                                          // 780
          serverDoc.writtenByStubs = {};                                                                          // 781
          serverDoc.writtenByStubs[methodId] = true;                                                              // 782
        }                                                                                                         // 783
      });                                                                                                         // 784
    });                                                                                                           // 785
    if (!_.isEmpty(docsWritten)) {                                                                                // 786
      self._documentsWrittenByStub[methodId] = docsWritten;                                                       // 787
    }                                                                                                             // 788
  },                                                                                                              // 789
                                                                                                                  // 790
  // This is very much a private function we use to make the tests                                                // 791
  // take up fewer server resources after they complete.                                                          // 792
  _unsubscribeAll: function () {                                                                                  // 793
    var self = this;                                                                                              // 794
    _.each(_.clone(self._subscriptions), function (sub, id) {                                                     // 795
      // Avoid killing the autoupdate subscription so that developers                                             // 796
      // still get hot code pushes when writing tests.                                                            // 797
      //                                                                                                          // 798
      // XXX it's a hack to encode knowledge about autoupdate here,                                               // 799
      // but it doesn't seem worth it yet to have a special API for                                               // 800
      // subscriptions to preserve after unit tests.                                                              // 801
      if (sub.name !== 'meteor_autoupdate_clientVersions') {                                                      // 802
        ...
        delete self._subscriptions[id];                                                                           // 804
                                                                                                                  ...
                                                                                                                  // 808
  // Sends the DDP stringification of the given message object                                                    // 809
  _send: function (obj) {                                                                                         // 810
    var self = this;                                                                                              // 811
    self._stream.send(stringifyDDP(obj));                                                                         // 812
  },                                                                                                              // 813
                                                                                                                  // 814
  status: function (/*passthrough args*/) {                                                                       // 815
    var self = this;                                                                                              // 816
    return self._stream.status.apply(self._stream, arguments);                                                    // 817
  },                                                                                                              // 818
                                                                                                                  // 819
  reconnect: function (/*passthrough args*/) {                                                                    // 820
    var self = this;                                                                                              // 821
    return self._stream.reconnect.apply(self._stream, arguments);                                                 // 822
  },                                                                                                              // 823
                                                                                                                  // 824
  disconnect: function (/*passthrough args*/) {                                                                   // 825
    var self = this;                                                                                              // 826
    return self._stream.disconnect.apply(self._stream, arguments);                                                // 827
  },                                                                                                              // 828
                                                                                                                  // 829
  close: function () {                                                                                            // 830
    var self = this;                                                                                              // 831
    return self._stream.disconnect({_permanent: true});                                                           // 832
  },                                                                                                              // 833
                                                                                                                  // 834
  ///                                                                                                             // 835
  /// Reactive user system                                                                                        // 836
  ///                                                                                                             // 837
  userId: function () {                                                                                           // 838
    var self = this;                                                                                              // 839
    if (self._userIdDeps)                                                                                         // 840
      self._userIdDeps.depend();                                                                                  // 841
    return self._userId;                                                                                          // 842
  },                                                                                                              // 843
                                                                                                                  // 844
  setUserId: function (userId) {                                                                                  // 845
    var self = this;                                                                                              // 846
    // Avoid invalidating dependents if setUserId is called with current value.                                   // 847
    if (self._userId === userId)                                                                                  // 848
      return;                                                                                                     // 849
    self._userId = userId;                                                                                        // 850
    if (self._userIdDeps)                                                                                         // 851
      self._userIdDeps.changed();                                                                                 // 852
  },                                                                                                              // 853
                                                                                                                  // 854
  // Returns true if we are in a state after reconnect of waiting for subs to be                                  // 855
  // revived or early methods to finish their data, or we are waiting for a                                       // 856
  // "wait" method to finish.                                                                                     // 857
  _waitingForQuiescence: function () {                                                                            // 858
    var self = this;                                                                                              // 859
    return (! _.isEmpty(self._subsBeingRevived) ||                                                                // 860
            ! _.isEmpty(self._methodsBlockingQuiescence));                                                        // 861
  },                                                                                                              // 862
                                                                                                                  // 863
  // Returns true if any method whose message has been sent to the server has                                     // 864
  // not yet invoked its user callback.                                                                           // 865
  _anyMethodsAreOutstanding: function () {                                                                        // 866
    var self = this;                                                                                              // 867
    return _.any(_.pluck(self._methodInvokers, 'sentMessage'));                                                   // 868
  },                                                                                                              // 869
                                                                                                                  // 870
  _livedata_connected: function (msg) {                                                                           // 871
    var self = this;                                                                                              // 872
                                                                                                                  // 873
    // If this is a reconnect, we'll have to reset all stores.                                                    // 874
    if (self._lastSessionId)                                                                                      // 875
      self._resetStores = true;                                                                                   // 876
                                                                                                                  // 877
    if (typeof (msg.session) === "string") {                                                                      // 878
      var reconnectedToPreviousSession = (self._lastSessionId === msg.session);                                   // 879
      self._lastSessionId = msg.session;                                                                          // 880
    }                                                                                                             // 881
                                                                                                                  // 882
    if (reconnectedToPreviousSession) {                                                                           // 883
      // Successful reconnection -- pick up where we left off.  Note that right                                   // 884
      // now, this never happens: the server never connects us to a previous                                      // 885
      // session, because DDP doesn't provide enough data for the server to know                                  // 886
      // what messages the client has processed. We need to improve DDP to make                                   // 887
      // this possible, at which point we'll probably need more code here.                                        // 888
      return;                                                                                                     // 889
    }                                                                                                             // 890
                                                                                                                  // 891
    // Server doesn't have our data any more. Re-sync a new session.                                              // 892
                                                                                                                  // 893
    // Forget about messages we were buffering for unknown collections. They'll                                   // 894
    // be resent if still relevant.                                                                               // 895
    self._updatesForUnknownStores = {};                                                                           // 896
                                                                                                                  // 897
    if (self._resetStores) {                                                                                      // 898
      // Forget about the effects of stubs. We'll be resetting all collections                                    // 899
      // anyway.                                                                                                  // 900
      self._documentsWrittenByStub = {};                                                                          // 901
      self._serverDocuments = {};                                                                                 // 902
    }                                                                                                             // 903
                                                                                                                  // 904
    // Clear _afterUpdateCallbacks.                                                                               // 905
    self._afterUpdateCallbacks = [];                                                                              // 906
                                                                                                                  // 907
    // Mark all named subscriptions which are ready (ie, we already called the                                    // 908
    // ready callback) as needing to be revived.                                                                  // 909
    // XXX We should also block reconnect quiescence until unnamed subscriptions                                  // 910
    //     (eg, autopublish) are done re-publishing to avoid flicker!                                             // 911
    self._subsBeingRevived = {};                                                                                  // 912
    _.each(self._subscriptions, function (sub, id) {                                                              // 913
      if (sub.ready)                                                                                              // 914
        self._subsBeingRevived[id] = true;                                                                        // 915
    });                                                                                                           // 916
                                                                                                                  // 917
    // Arrange for "half-finished" methods to have their callbacks run, and                                       // 918
    // track methods that were sent on this connection so that we don't                                           // 919
    // quiesce until they are all done.                                                                           // 920
    //                                                                                                            // 921
    // Start by clearing _methodsBlockingQuiescence: methods sent before                                          // 922
    // reconnect don't matter, and any "wait" methods sent on the new connection                                  // 923
    // that we drop here will be restored by the loop below.                                                      // 924
    self._methodsBlockingQuiescence = {};                                                                         // 925
    if (self._resetStores) {                                                                                      // 926
      _.each(self._methodInvokers, function (invoker) {                                                           // 927
        if (invoker.gotResult()) {                                                                                // 928
          // This method already got its result, but it didn't call its callback                                  // 929
          // because its data didn't become visible. We did not resend the                                        // 930
          // method RPC. We'll call its callback when we get a full quiesce,                                      // 931
          // since that's as close as we'll get to "data must be visible".                                        // 932
          self._afterUpdateCallbacks.push(_.bind(invoker.dataVisible, invoker));                                  // 933
        } else if (invoker.sentMessage) {                                                                         // 934
          // This method has been sent on this connection (maybe as a resend                                      // 935
          // from the last connection, maybe from onReconnect, maybe just very                                    // 936
          // quickly before processing the connected message).                                                    // 937
          //                                                                                                      // 938
          // We don't need to do anything special to ensure its callbacks get                                     // 939
          // called, but we'll count it as a method which is preventing                                           // 940
          // reconnect quiescence. (eg, it might be a login method that was run                                   // 941
          // from onReconnect, and we don't want to see flicker by seeing a                                       // 942
          // logged-out state.)                                                                                   // 943
          self._methodsBlockingQuiescence[invoker.methodId] = true;                                               // 944
        }                                                                                                         // 945
      });                                                                                                         // 946
    }                                                                                                             // 947
                                                                                                                  // 948
    self._messagesBufferedUntilQuiescence = [];                                                                   // 949
                                                                                                                  // 950
    // If we're not waiting on any methods or subs, we can reset the stores and                                   // 951
    // call the callbacks immediately.                                                                            // 952
    if (!self._waitingForQuiescence()) {                                                                          // 953
      if (self._resetStores) {                                                                                    // 954
        _.each(self._stores, function (s) {                                                                       // 955
          s.beginUpdate(0, true);                                                                                 // 956
          s.endUpdate();                                                                                          // 957
        });                                                                                                       // 958
        self._resetStores = false;                                                                                // 959
      }                                                                                                           // 960
      self._runAfterUpdateCallbacks();                                                                            // 961
    }                                                                                                             // 962
  },                                                                                                              // 963
                                                                                                                  // 964
                                                                                                                  // 965
  _processOneDataMessage: function (msg, updates) {                                                               // 966
    var self = this;                                                                                              // 967
    // Using underscore here so as not to need to capitalize.                                                     // 968
    self['_process_' + msg.msg](msg, updates);                                                                    // 969
  },                                                                                                              // 970
                                                                                                                  // 971
                                                                                                                  // 972
  _livedata_data: function (msg) {                                                                                // 973
    var self = this;                                                                                              // 974
                                                                                                                  // 975
    // collection name -> array of messages                                                                       // 976
    var updates = {};                                                                                             // 977
                                                                                                                  // 978
    if (self._waitingForQuiescence()) {                                                                           // 979
      self._messagesBufferedUntilQuiescence.push(msg);                                                            // 980
                                                                                                                  // 981
      if (msg.msg === "nosub")                                                                                    // 982
        delete self._subsBeingRevived[msg.id];                                                                    // 983
                                                                                                                  // 984
      _.each(msg.subs || [], function (subId) {                                                                   // 985
        delete self._subsBeingRevived[subId];                                                                     // 986
      });                                                                                                         // 987
      _.each(msg.methods || [], function (methodId) {                                                             // 988
        delete self._methodsBlockingQuiescence[methodId];                                                         // 989
      });                                                                                                         // 990
                                                                                                                  // 991
      if (self._waitingForQuiescence())                                                                           // 992
        return;                                                                                                   // 993
                                                                                                                  // 994
      // No methods or subs are blocking quiescence!                                                              // 995
      // We'll now process and all of our buffered messages, reset all stores,                                    // 996
      // and apply them all at once.                                                                              // 997
      _.each(self._messagesBufferedUntilQuiescence, function (bufferedMsg) {                                      // 998
        self._processOneDataMessage(bufferedMsg, updates);                                                        // 999
      });                                                                                                         // 1000
      self._messagesBufferedUntilQuiescence = [];                                                                 // 1001
    } else {                                                                                                      // 1002
      self._processOneDataMessage(msg, updates);                                                                  // 1003
    }                                                                                                             // 1004
                                                                                                                  // 1005
    if (self._resetStores || !_.isEmpty(updates)) {                                                               // 1006
      // Begin a transactional update of each store.                                                              // 1007
      _.each(self._stores, function (s, storeName) {                                                              // 1008
        s.beginUpdate(_.has(updates, storeName) ? updates[storeName].length : 0,                                  // 1009
                      self._resetStores);                                                                         // 1010
      });                                                                                                         // 1011
      self._resetStores = false;                                                                                  // 1012
                                                                                                                  // 1013
      _.each(updates, function (updateMessages, storeName) {                                                      // 1014
        var store = self._stores[storeName];                                                                      // 1015
        if (store) {                                                                                              // 1016
          _.each(updateMessages, function (updateMessage) {                                                       // 1017
            store.update(updateMessage);                                                                          // 1018
          });                                                                                                     // 1019
        } else {                                                                                                  // 1020
          // Nobody's listening for this data. Queue it up until                                                  // 1021
          // someone wants it.                                                                                    // 1022
          // XXX memory use will grow without bound if you forget to                                              // 1023
          // create a collection or just don't care about it... going                                             // 1024
          // to have to do something about that.                                                                  // 1025
          if (!_.has(self._updatesForUnknownStores, storeName))                                                   // 1026
            self._updatesForUnknownStores[storeName] = [];                                                        // 1027
          Array.prototype.push.apply(self._updatesForUnknownStores[storeName],                                    // 1028
                                     updateMessages);                                                             // 1029
        }                                                                                                         // 1030
      });                                                                                                         // 1031
                                                                                                                  // 1032
      // End update transaction.                                                                                  // 1033
      _.each(self._stores, function (s) { s.endUpdate(); });                                                      // 1034
    }                                                                                                             // 1035
                                                                                                                  // 1036
    self._runAfterUpdateCallbacks();                                                                              // 1037
  },                                                                                                              // 1038
                                                                                                                  // 1039
  // Call any callbacks deferred with _runWhenAllServerDocsAreFlushed whose                                       // 1040
  // relevant docs have been flushed, as well as dataVisible callbacks at                                         // 1041
  // reconnect-quiescence time.                                                                                   // 1042
  _runAfterUpdateCallbacks: function () {                                                                         // 1043
    var self = this;                                                                                              // 1044
    var callbacks = self._afterUpdateCallbacks;                                                                   // 1045
    self._afterUpdateCallbacks = [];                                                                              // 1046
    _.each(callbacks, function (c) {                                                                              // 1047
      c();                                                                                                        // 1048
    });                                                                                                           // 1049
  },                                                                                                              // 1050
                                                                                                                  // 1051
  _pushUpdate: function (updates, collection, msg) {                                                              // 1052
    var self = this;                                                                                              // 1053
    if (!_.has(updates, collection)) {                                                                            // 1054
      updates[collection] = [];                                                                                   // 1055
    }                                                                                                             // 1056
    updates[collection].push(msg);                                                                                // 1057
  },                                                                                                              // 1058
                                                                                                                  // 1059
  _getServerDoc: function (collection, id) {                                                                      // 1060
    var self = this;                                                                                              // 1061
    if (!_.has(self._serverDocuments, collection))                                                                // 1062
      return null;                                                                                                // 1063
    var serverDocsForCollection = self._serverDocuments[collection];                                              // 1064
    return serverDocsForCollection.get(id) || null;                                                               // 1065
  },                                                                                                              // 1066
                                                                                                                  // 1067
  _process_added: function (msg, updates) {                                                                       // 1068
    var self = this;                                                                                              // 1069
    var id = LocalCollection._idParse(msg.id);                                                                    // 1070
    var serverDoc = self._getServerDoc(msg.collection, id);                                                       // 1071
    if (serverDoc) {                                                                                              // 1072
      // Some outstanding stub wrote here.                                                                        // 1073
      if (serverDoc.document !== undefined)                                                                       // 1074
        throw new Error("Server sent add for existing id: " + msg.id);                                            // 1075
      serverDoc.document = msg.fields || {};                                                                      // 1076
      serverDoc.document._id = id;                                                                                // 1077
    } else {                                                                                                      // 1078
      self._pushUpdate(updates, msg.collection, msg);                                                             // 1079
    }                                                                                                             // 1080
  },                                                                                                              // 1081
                                                                                                                  // 1082
  _process_changed: function (msg, updates) {                                                                     // 1083
    var self = this;                                                                                              // 1084
    var serverDoc = self._getServerDoc(                                                                           // 1085
      msg.collection, LocalCollection._idParse(msg.id));                                                          // 1086
    if (serverDoc) {                                                                                              // 1087
      if (serverDoc.document === undefined)                                                                       // 1088
        throw new Error("Server sent changed for nonexisting id: " + msg.id);                                     // 1089
      LocalCollection._applyChanges(serverDoc.document, msg.fields);                                              // 1090
    } else {                                                                                                      // 1091
      self._pushUpdate(updates, msg.collection, msg);                                                             // 1092
    }                                                                                                             // 1093
  },                                                                                                              // 1094
                                                                                                                  // 1095
  _process_removed: function (msg, updates) {                                                                     // 1096
    var self = this;                                                                                              // 1097
    var serverDoc = self._getServerDoc(                                                                           // 1098
      msg.collection, LocalCollection._idParse(msg.id));                                                          // 1099
    if (serverDoc) {                                                                                              // 1100
      // Some outstanding stub wrote here.                                                                        // 1101
      if (serverDoc.document === undefined)                                                                       // 1102
        throw new Error("Server sent removed for nonexisting id:" + msg.id);                                      // 1103
      serverDoc.document = undefined;                                                                             // 1104
    } else {                                                                                                      // 1105
      self._pushUpdate(updates, msg.collection, {                                                                 // 1106
        msg: 'removed',                                                                                           // 1107
        collection: msg.collection,                                                                               // 1108
        id: msg.id                                                                                                // 1109
      });                                                                                                         // 1110
    }                                                                                                             // 1111
  },                                                                                                              // 1112
                                                                                                                  // 1113
  _process_updated: function (msg, updates) {                                                                     // 1114
    var self = this;                                                                                              // 1115
    // Process "method done" messages.                                                                            // 1116
    _.each(msg.methods, function (methodId) {                                                                     // 1117
      _.each(self._documentsWrittenByStub[methodId], function (written) {                                         // 1118
        var serverDoc = self._getServerDoc(written.collection, written.id);                                       // 1119
        if (!serverDoc)                                                                                           // 1120
          throw new Error("Lost serverDoc for " + JSON.stringify(written));                                       // 1121
        if (!serverDoc.writtenByStubs[methodId])                                                                  // 1122
          throw new Error("Doc " + JSON.stringify(written) +                                                      // 1123
                          " not written by  method " + methodId);                                                 // 1124
        delete serverDoc.writtenByStubs[methodId];                                                                // 1125
        if (_.isEmpty(serverDoc.writtenByStubs)) {                                                                // 1126
          // All methods whose stubs wrote this method have completed! We can                                     // 1127
          // now copy the saved document to the database (reverting the stub's                                    // 1128
          // change if the server did not write to this object, or applying the                                   // 1129
          // server's writes if it did).                                                                          // 1130
                                                                                                                  // 1131
          // This is a fake ddp 'replace' message.  It's just for talking                                         // 1132
          // between livedata connections and minimongo.  (We have to stringify                                   // 1133
          // the ID because it's supposed to look like a wire message.)                                           // 1134
          self._pushUpdate(updates, written.collection, {                                                         // 1135
            msg: 'replace',                                                                                       // 1136
            id: LocalCollection._idStringify(written.id),                                                         // 1137
            replace: serverDoc.document                                                                           // 1138
          });                                                                                                     // 1139
          // Call all flush callbacks.                                                                            // 1140
          _.each(serverDoc.flushCallbacks, function (c) {                                                         // 1141
            c();                                                                                                  // 1142
          });                                                                                                     // 1143
                                                                                                                  // 1144
          // Delete this completed serverDocument. Don't bother to GC empty                                       // 1145
          // IdMaps inside self._serverDocuments, since there probably aren't                                     // 1146
          // many collections and they'll be written repeatedly.                                                  // 1147
          self._serverDocuments[written.collection].remove(written.id);                                           // 1148
        }                                                                                                         // 1149
      });                                                                                                         // 1150
      delete self._documentsWrittenByStub[methodId];                                                              // 1151
                                                                                                                  // 1152
      // We want to call the data-written callback, but we can't do so until all                                  // 1153
      // currently buffered messages are flushed.                                                                 // 1154
      var callbackInvoker = self._methodInvokers[methodId];                                                       // 1155
      if (!callbackInvoker)                                                                                       // 1156
        throw new Error("No callback invoker for method " + methodId);                                            // 1157
      self._runWhenAllServerDocsAreFlushed(                                                                       // 1158
        _.bind(callbackInvoker.dataVisible, callbackInvoker));                                                    // 1159
    });                                                                                                           // 1160
  },                                                                                                              // 1161
                                                                                                                  // 1162
  _process_ready: function (msg, updates) {                                                                       // 1163
    var self = this;                                                                                              // 1164
    // Process "sub ready" messages. "sub ready" messages don't take effect                                       // 1165
    // until all current server documents have been flushed to the local                                          // 1166
    // database. We can use a write fence to implement this.                                                      // 1167
    _.each(msg.subs, function (subId) {                                                                           // 1168
      self._runWhenAllServerDocsAreFlushed(function () {                                                          // 1169
        var subRecord = self._subscriptions[subId];                                                               // 1170
        // Did we already unsubscribe?                                                                            // 1171
        if (!subRecord)                                                                                           // 1172
          return;                                                                                                 // 1173
        // Did we already receive a ready message? (Oops!)                                                        // 1174
        if (subRecord.ready)                                                                                      // 1175
          return;                                                                                                 // 1176
        subRecord.readyCallback && subRecord.readyCallback();                                                     // 1177
        subRecord.ready = true;                                                                                   // 1178
        subRecord.readyDeps && subRecord.readyDeps.changed();                                                     // 1179
      });                                                                                                         // 1180
    });                                                                                                           // 1181
  },                                                                                                              // 1182
                                                                                                                  // 1183
  // Ensures that "f" will be called after all documents currently in                                             // 1184
  // _serverDocuments have been written to the local cache. f will not be called                                  // 1185
  // if the connection is lost before then!                                                                       // 1186
  _runWhenAllServerDocsAreFlushed: function (f) {                                                                 // 1187
    var self = this;                                                                                              // 1188
    var runFAfterUpdates = function () {                                                                          // 1189
      self._afterUpdateCallbacks.push(f);                                                                         // 1190
    };                                                                                                            // 1191
    var unflushedServerDocCount = 0;                                                                              // 1192
    var onServerDocFlush = function () {                                                                          // 1193
      --unflushedServerDocCount;                                                                                  // 1194
      if (unflushedServerDocCount === 0) {                                                                        // 1195
        // This was the last doc to flush! Arrange to run f after the updates                                     // 1196
        // have been applied.                                                                                     // 1197
        runFAfterUpdates();                                                                                       // 1198
      }                                                                                                           // 1199
    };                                                                                                            // 1200
    _.each(self._serverDocuments, function (collectionDocs) {                                                     // 1201
      collectionDocs.forEach(function (serverDoc) {                                                               // 1202
        var writtenByStubForAMethodWithSentMessage = _.any(                                                       // 1203
          serverDoc.writtenByStubs, function (dummy, methodId) {                                                  // 1204
            var invoker = self._methodInvokers[methodId];                                                         // 1205
            return invoker && invoker.sentMessage;                                                                // 1206
          });                                                                                                     // 1207
        if (writtenByStubForAMethodWithSentMessage) {                                                             // 1208
          ++unflushedServerDocCount;                                                                              // 1209
          serverDoc.flushCallbacks.push(onServerDocFlush);                                                        // 1210
        }                                                                                                         // 1211
      });                                                                                                         // 1212
    });                                                                                                           // 1213
    if (unflushedServerDocCount === 0) {                                                                          // 1214
      // There aren't any buffered docs --- we can call f as soon as the current                                  // 1215
      // round of updates is applied!                                                                             // 1216
      runFAfterUpdates();                                                                                         // 1217
    }                                                                                                             // 1218
  },                                                                                                              // 1219
                                                                                                                  // 1220
  _livedata_nosub: function (msg) {                                                                               // 1221
    var self = this;                                                                                              // 1222
                                                                                                                  // 1223
    // First pass it through _livedata_data, which only uses it to help get                                       // 1224
    // towards quiescence.                                                                                        // 1225
    self._livedata_data(msg);                                                                                     // 1226
                                                                                                                  // 1227
    // Do the rest of our processing immediately, with no                                                         // 1228
    // buffering-until-quiescence.                                                                                // 1229
                                                                                                                  // 1230
    // we weren't subbed anyway, or we initiated the unsub.                                                       // 1231
    if (!_.has(self._subscriptions, msg.id))                                                                      // 1232
      return;                                                                                                     // 1233
    var errorCallback = self._subscriptions[msg.id].errorCallback;                                                // 1234
    delete self._subscriptions[msg.id];                                                                           // 1235
    if (errorCallback && msg.error) {                                                                             // 1236
      errorCallback(new Meteor.Error(                                                                             // 1237
        msg.error.error, msg.error.reason, msg.error.details));                                                   // 1238
    }                                                                                                             // 1239
  },                                                                                                              // 1240
                                                                                                                  // 1241
  _process_nosub: function () {                                                                                   // 1242
    // This is called as part of the "buffer until quiescence" process, but                                       // 1243
    // nosub's effect is always immediate. It only goes in the buffer at all                                      // 1244
    // because it's possible for a nosub to be the thing that triggers                                            // 1245
    // quiescence, if we were waiting for a sub to be revived and it dies                                         // 1246
    // instead.                                                                                                   // 1247
  },                                                                                                              // 1248
                                                                                                                  // 1249
  _livedata_result: function (msg) {                                                                              // 1250
    // id, result or error. error has error (code), reason, details                                               // 1251
                                                                                                                  // 1252
    var self = this;                                                                                              // 1253
                                                                                                                  // 1254
    // find the outstanding request                                                                               // 1255
    // should be O(1) in nearly all realistic use cases                                                           // 1256
    if (_.isEmpty(self._outstandingMethodBlocks)) {                                                               // 1257
      Meteor._debug("Received method result but no methods outstanding");                                         // 1258
      return;                                                                                                     // 1259
    }                                                                                                             // 1260
    var currentMethodBlock = self._outstandingMethodBlocks[0].methods;                                            // 1261
    var m;                                                                                                        // 1262
    for (var i = 0; i < currentMethodBlock.length; i++) {                                                         // 1263
      m = currentMethodBlock[i];                                                                                  // 1264
      if (m.methodId === msg.id)                                                                                  // 1265
        break;                                                                                                    // 1266
    }                                                                                                             // 1267
                                                                                                                  // 1268
    if (!m) {                                                                                                     // 1269
      Meteor._debug("Can't match method response to original method call", msg);                                  // 1270
      return;                                                                                                     // 1271
    }                                                                                                             // 1272
                                                                                                                  // 1273
    // Remove from current method block. This may leave the block empty, but we                                   // 1274
    // don't move on to the next block until the callback has been delivered, in                                  // 1275
    // _outstandingMethodFinished.                                                                                // 1276
    currentMethodBlock.splice(i, 1);                                                                              // 1277
                                                                                                                  // 1278
    if (_.has(msg, 'error')) {                                                                                    // 1279
      m.receiveResult(new Meteor.Error(                                                                           // 1280
        msg.error.error, msg.error.reason,                                                                        // 1281
        msg.error.details));                                                                                      // 1282
    } else {                                                                                                      // 1283
      // msg.result may be undefined if the method didn't return a                                                // 1284
      // value                                                                                                    // 1285
      m.receiveResult(undefined, msg.result);                                                                     // 1286
    }                                                                                                             // 1287
  },                                                                                                              // 1288
                                                                                                                  // 1289
  // Called by MethodInvoker after a method's callback is invoked.  If this was                                   // 1290
  // the last outstanding method in the current block, runs the next block. If                                    // 1291
  // there are no more methods, consider accepting a hot code push.                                               // 1292
  _outstandingMethodFinished: function () {                                                                       // 1293
    var self = this;                                                                                              // 1294
    if (self._anyMethodsAreOutstanding())                                                                         // 1295
      return;                                                                                                     // 1296
                                                                                                                  // 1297
    // No methods are outstanding. This should mean that the first block of                                       // 1298
    // methods is empty. (Or it might not exist, if this was a method that                                        // 1299
    // half-finished before disconnect/reconnect.)                                                                // 1300
    if (! _.isEmpty(self._outstandingMethodBlocks)) {                                                             // 1301
      var firstBlock = self._outstandingMethodBlocks.shift();                                                     // 1302
      if (! _.isEmpty(firstBlock.methods))                                                                        // 1303
        throw new Error("No methods outstanding but nonempty block: " +                                           // 1304
                        JSON.stringify(firstBlock));                                                              // 1305
                                                                                                                  // 1306
      // Send the outstanding methods now in the first block.                                                     // 1307
      if (!_.isEmpty(self._outstandingMethodBlocks))                                                              // 1308
        self._sendOutstandingMethods();                                                                           // 1309
    }                                                                                                             // 1310
                                                                                                                  // 1311
    // Maybe accept a hot code push.                                                                              // 1312
    self._maybeMigrate();                                                                                         // 1313
  },                                                                                                              // 1314
                                                                                                                  // 1315
  // Sends messages for all the methods in the first block in                                                     // 1316
  // _outstandingMethodBlocks.                                                                                    // 1317
  _sendOutstandingMethods: function() {                                                                           // 1318
    var self = this;                                                                                              // 1319
    if (_.isEmpty(self._outstandingMethodBlocks))                                                                 // 1320
      return;                                                                                                     // 1321
    _.each(self._outstandingMethodBlocks[0].methods, function (m) {                                               // 1322
      m.sendMessage();                                                                                            // 1323
    });                                                                                                           // 1324
  },                                                                                                              // 1325
                                                                                                                  // 1326
  _livedata_error: function (msg) {                                                                               // 1327
    Meteor._debug("Received error from server: ", msg.reason);                                                    // 1328
    if (msg.offendingMessage)                                                                                     // 1329
      Meteor._debug("For: ", msg.offendingMessage);                                                               // 1330
  },                                                                                                              // 1331
                                                                                                                  // 1332
  _callOnReconnectAndSendAppropriateOutstandingMethods: function() {                                              // 1333
    var self = this;                                                                                              // 1334
    var oldOutstandingMethodBlocks = self._outstandingMethodBlocks;                                               // 1335
    self._outstandingMethodBlocks = [];                                                                           // 1336
                                                                                                                  // 1337
    self.onReconnect();                                                                                           // 1338
                                                                                                                  // 1339
    if (_.isEmpty(oldOutstandingMethodBlocks))                                                                    // 1340
      return;                                                                                                     // 1341
                                                                                                                  // 1342
    // We have at least one block worth of old outstanding methods to try                                         // 1343
    // again. First: did onReconnect actually send anything? If not, we just                                      // 1344
    // restore all outstanding methods and run the first block.                                                   // 1345
    if (_.isEmpty(self._outstandingMethodBlocks)) {                                                               // 1346
      self._outstandingMethodBlocks = oldOutstandingMethodBlocks;                                                 // 1347
      self._sendOutstandingMethods();                                                                             // 1348
      return;                                                                                                     // 1349
    }                                                                                                             // 1350
                                                                                                                  // 1351
    // OK, there are blocks on both sides. Special case: merge the last block of                                  // 1352
    // the reconnect methods with the first block of the original methods, if                                     // 1353
    // neither of them are "wait" blocks.                                                                         // 1354
    if (!_.last(self._outstandingMethodBlocks).wait &&                                                            // 1355
        !oldOutstandingMethodBlocks[0].wait) {                                                                    // 1356
      _.each(oldOutstandingMethodBlocks[0].methods, function (m) {                                                // 1357
        _.last(self._outstandingMethodBlocks).methods.push(m);                                                    // 1358
                                                                                                                  // 1359
        // If this "last block" is also the first block, send the message.                                        // 1360
        if (self._outstandingMethodBlocks.length === 1)                                                           // 1361
          m.sendMessage();                                                                                        // 1362
      });                                                                                                         // 1363
                                                                                                                  // 1364
      oldOutstandingMethodBlocks.shift();                                                                         // 1365
    }                                                                                                             // 1366
                                                                                                                  // 1367
    // Now add the rest of the original blocks on.                                                                // 1368
    _.each(oldOutstandingMethodBlocks, function (block) {                                                         // 1369
      self._outstandingMethodBlocks.push(block);                                                                  // 1370
    });                                                                                                           // 1371
  },                                                                                                              // 1372
                                                                                                                  // 1373
  // We can accept a hot code push if there are no methods in flight.                                             // 1374
  _readyToMigrate: function() {                                                                                   // 1375
    var self = this;                                                                                              // 1376
    return _.isEmpty(self._methodInvokers);                                                                       // 1377
  },                                                                                                              // 1378
                                                                                                                  // 1379
  // If we were blocking a migration, see if it's now possible to continue.                                       // 1380
  // Call whenever the set of outstanding/blocked methods shrinks.                                                // 1381
  _maybeMigrate: function () {                                                                                    // 1382
    var self = this;                                                                                              // 1383
    if (self._retryMigrate && self._readyToMigrate()) {                                                           // 1384
      self._retryMigrate();                                                                                       // 1385
      self._retryMigrate = null;                                                                                  // 1386
    }                                                                                                             // 1387
  }                                                                                                               // 1388
});                                                                                                               // 1389
                                                                                                                  // 1390
LivedataTest.Connection = Connection;                                                                             // 1391
                                                                                                                  // 1392
// @param url {String} URL to Meteor app,                                                                         // 1393
//     e.g.:                                                                                                      // 1394
//     "subdomain.meteor.com",                                                                                    // 1395
//     "http://subdomain.meteor.com",                                                                             // 1396
//     "/",                                                                                                       // 1397
//     "ddp+sockjs://ddp--****-foo.meteor.com/sockjs"                                                             // 1398
//                                                                                                                // 1399
DDP.connect = function (url, options) {                                                                           // 1400
  var ret = new Connection(url, options);                                                                         // 1401
  allConnections.push(ret); // hack. see below.                                                                   // 1402
  return ret;                                                                                                     // 1403
};                                                                                                                // 1404
                                                                                                                  // 1405
// Hack for `spiderable` package: a way to see if the page is done                                                // 1406
// loading all the data it needs.                                                                                 // 1407
//                                                                                                                // 1408
allConnections = [];                                                                                              // 1409
DDP._allSubscriptionsReady = function () {                                                                        // 1410
  return _.all(allConnections, function (conn) {                                                                  // 1411
    return _.all(conn._subscriptions, function (sub) {                                                            // 1412
      return sub.ready;                                                                                           // 1413
    });                                                                                                           // 1414
  });                                                                                                             // 1415
};                                                                                                                // 1416
                                                                                                                  // 1417
...
//# sourceMappingURL=bdd3590efbcd8c64952cdafd345700a3fef2b4e6.map
